  ...
  return (
    ...
    >
      <div className='w-full flex justify-between items-center max-w-7xl mx-auto'>
        ...
        <div className='sm:hidden flex flex-1 justify-end items-center text-black-100'>
          <img
          ...
          >
            <ul className='list-none flex justify-end items-start flex-1 flex-col gap-4'>
              {navLinks.map((nav) => (
                <li
                  key={nav.id}
                  className={`font-poppins font-medium cursor-pointer text-[16px] ${active === nav.title ? "text-white-100" : "text-white"
                    }`}
                  onClick={() => {
                    setToggle(!toggle);
                    setActive(nav.title);
